      ...
      <Link href={`/portfolio/${company.slug}`} className="block no-underline group">
        <div className="bg-white rounded-[20px] overflow-hidden border border-navy/[0.06] transition-all duration-400 ease-out-expo hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(46,62,111,0.1)]">
          <div className="px-8 pt-8 pb-5 flex items-start gap-5">
            <div
              className={`w-14 h-14 rounded-[14px] bg-gradient-to-br ${company.logoGradient} flex items-center justify-center flex-shrink-0 font-serif text-[1.4rem] font-bold text-white`}
            >
              {company.logoLetter}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-[1.4rem] text-navy font-bold">
                  {company.name}
                  {company.nameAr && (
                    <span className="font-sans text-[0.85rem] text-text-muted font-normal ml-2">
                      / {company.nameAr}
                    </span>
                  )}
                </h3>
